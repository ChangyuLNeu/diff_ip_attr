#we don't use this python file.
#our imagen_Pytorch is modified from this file
import math
import copy
...
class Unet(nn.Module):
    ...
    def forward(
        ...
        *,
        ...
        if exists(text_embeds) and self.cond_on_text:                                                   #text embeds transferred to  text_hiddens(added to t) and text_tokens (concat with time_tokens)

            # conditional dropout

            text_keep_mask = prob_mask_like((batch_size,), 1 - cond_drop_prob, device = device)        #4

            text_keep_mask_embed = rearrange(text_keep_mask, 'b -> b 1 1')                             #4*1*1      
            text_keep_mask_hidden = rearrange(text_keep_mask, 'b -> b 1')                               #4*1

            # calculate text embeds

            text_tokens = self.text_to_cond(text_embeds)                                               #4 * 256 * 768 -> 4 * 256 * 512
        ...
        c = time_tokens if not exists(text_tokens) else torch.cat((time_tokens, text_tokens), dim = -2)       #4*2*512, 4 * 36 * 512 -> 4 * 36 * 512      
